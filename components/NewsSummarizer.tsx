...
const NewsSummarizer: React.FC = () => {
    const [topic, setTopic] = useState<string>('latest GDPR rulings');
    ...
    const handleSummarize = useCallback(async () => {
        ...
        try {
            const response = await summarizeLegalNews(topic);
            setResult(response);
        } catch (err) {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 bg-slate-800/50 border border-slate-700 rounded-lg shadow-2xl">
            <div className="text-center mb-6">
                ...
                <h2 className="mt-2 text-2xl font-semibold text-slate-100">Legal News Summarizer</h2>
                <p className="mt-1 text-sm text-slate-400">Get AI-powered summaries of the latest legal news and rulings, with sources.</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-6">
                <div className="relative flex-grow w-full sm:w-auto">
                    <input
                        ...
                        placeholder="e.g., recent data privacy laws"
                ...
                >
                    {isLoading ? <LoadingIcon className="w-5 h-5"/> : <><SearchIcon className="w-5 h-5" /><span>Summarize</span></>}
                </button>
            </div>
            ...
            {result && (
                <div className="mt-6 space-y-6">
                    <div>
                        <h3 className="text-lg font-semibold text-slate-100 mb-2">Summary:</h3>
