...
const getStatusColor = (status: Experiment['status']) => {
    switch (status) {
        ...
        case 'In Development': return 'bg-[rgba(var(--primary-rgb),0.2)] text-dynamic border-[color:var(--primary-color)] opacity-70';
    ...
    return (
        <div className="max-w-5xl w-full mx-auto p-4 sm:p-6 bg-slate-800/50 border border-slate-700 rounded-lg shadow-2xl">
            <div className="text-center mb-8">
                <FlaskIcon className="mx-auto h-12 w-12 text-dynamic" />
                    ...
                    <div key={exp.id} className="p-6 bg-slate-900 rounded-lg border border-slate-700 flex flex-col">
                        <div className="flex-grow">
                            <div className="flex justify-between items-start">
                                <h3 className="text-lg font-semibold text-dynamic">{exp.title}</h3>
